    ...
    return (
        <div style={styles.card}>
            <p>
                ...
                <strong> ${formatNumber(formData?.monthly_contribution)}</strong> each month, or roughly
                <strong> {formatNumber(data.total_btc)}</strong> every year into a College Savings 529 Plan, earning
                <strong> {formatNumber(formData?.cagr)}</strong> compound growth annually, after {year} years you would have roughly
                <strong> ${formatNumber(data.btc_net_value)}</strong>. Comparatively, putting the same amount into Bitcoin appreciating at
                <strong> {cagrValue}</strong> annually, produces an ending net value of
                <strong> ${formatNumber(data.btc_net_value)}</strong>
                {/*, a difference of*/}
